...
if(count($errors)==0){
    ...
    $StyID = $_REQUEST['StyID'];
	$sql = "INSERT INTO  bookapp (AppoID, Date, Time, customerID,StyID) VALUES ('$AppoID','$Date','$Time','$userprofile','$StyID') ";
...
if (isset($_POST['cancel'])) {
 ...
 if (count($errors)==0) {
	...
	$query5="DELETE FROM bookapp WHERE AppoID='$AppoID2' AND customerID=('$userprofile') ";
...
if (empty($addname)) {
	array_push($errors,"Stylist Name is required");
	# code...
}
...
if(count($errors)==0){
		...
		$addService 	= $_REQUEST['addService'];


	$sqladd = "INSERT INTO  stylist (StylistID, Stylistname, email, Address, ContactNumber, Password,services) VALUES ('$addID','$addname','$addEmail','$addAddress','$addContactNumber','$addPassword','$addService') ";
...
if (isset($_POST['Delete'])) {
 ...
 if (count($errors)==0) {
	...
	$querydelete="DELETE FROM stylist WHERE StylistID='$deleteID' ";
	if ($mysqli -> query($querydelete)) {

		if ($mysqli->affected_rows==0) {
			 array_push($errors,"Wrong Stylist ID");
...
<!-- $query1="SELECT services FROM stylist";
   $result1= mysqli_query($mysqli, $query1);
   $opt="<select name='Categorey'>";
   while ($row= mysqli_fetch_assoc($result1)) {
   	$opt.="<option value ='{$row['services']}'>{$row['services']}</option>";
   }
   	$opt.="</select>";
